program gen_xdmf
!
! this program generates a XDMF file for visualizing the IBM scalar fields.
!
! usage:
! (1) set the parameters in the file 'param_solid.h90'
! (2) compile and run sequentially gen_grid.f90 and gen_xdmf_solid.f90 
!     (see genview.sh)
! (3) visualize the flow with the output file viewfld_DNS_solid.xmf
!     e.g. with paraview: paraview viewfld_DNS_solid.xmf
! it assumes: 
!        - visualization files have the format:
!          'XXXX_fld_YYYYYYY.bin'
!          where XXXX is the name of the field set in param.h90 and 
...
indent = 0
open(unit =ixdmf, file = 'viewfld_'//trim(casename)//'.xdmf',form='formatted')
